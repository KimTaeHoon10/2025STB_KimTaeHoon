...
#5.address변수
foodshop$district<-substr(foodshop$address,5,8)#시정보를 분리하여 변수 생성
...
ggplot()+
  geom_line(data=open_close_trend, aes(year,open_n,color="개업"))+#개업그래프
  geom_line(data=open_close_trend, aes(year,close_n,color="폐업"))+#폐업그래프
  xlab("연도") + ylab("개수")
